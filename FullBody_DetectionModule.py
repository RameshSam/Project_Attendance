...
def main():
    ...
    while True :
        ...
        img , facemesh_data = dector.findFaceMesh(img)
        img , facedetect_data = dector.finddetectface(img)

        # Body = dector.getposition(img)

        ctime = time.time()
        fps = 1 / (ctime - ptime)
        cv2.putText(img,f" FPS : {int(fps)}",(20,50),cv2.FONT_HERSHEY_PLAIN,4,(255,0,255),4)
        ptime =  ctime

        resize = cv2.resize(img,(1000,600))
        # if not resize[0] :
            # print("resize extented ")
